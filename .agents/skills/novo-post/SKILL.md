...
8. **Assemble `_posts/<date>-<slug>.md`.**
   ...
   - Body sections, in order (this matches every post since 2024 — skip "Parti-gyle" unless `notas.md` mentions a second brew off the same mash):
     ...
     7. `### ⚖️ Comparativo: Estimado (receita) × Medido (lote)` — `tables.comparativo_estimado`. Leave FG/ABV/IBU/EBC "Medido" as `—` (they need fermentation to finish). **OG is the exception**: if `notas.md` states a measured OG (it's known same brew day, e.g. "OG medida no fim: 1.048"), fill it in now instead of leaving it `—`.
